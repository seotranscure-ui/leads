...
import { isHigh } from './leads'
import { monthKeyOf } from './time'

export interface MonthStat { key: string; label: string; leads: number; demos: number; sales: number; ht: number }
...
export function monthlyStats(leads: Lead[], rule: HighTicketRule, filter?: (l: Lead) => boolean): MonthStat[] {
  ...
  for (const l of leads) {
    ...
    if (!map[key]) map[key] = { key, label, leads: 0, demos: 0, sales: 0, ht: 0 }
    map[key].leads++
    if (isDemo(l.stage)) map[key].demos++
    if (isWon(l.stage)) map[key].sales++
